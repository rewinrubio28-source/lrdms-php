    ...
    if ($formAction === 'update_profile') {
        ...
        if (empty($user['email'])) {
            ...
            <body>
                <div class="container">
                    ...
                    <div class="content">
                        ...
                        <p>Use the code below to confirm and enable two-factor authentication on your LRDMS account:</p>
                        <p style="text-align: center;">
                            <span class="code-box">' . htmlspecialchars($otpCode) . '</span>
                        </p>
                        <p>This code will expire in 10 minutes.</p>
                        <p>If you did not request this, you can ignore this email — 2FA will not be enabled.</p>
                    </div>
                    <div class="footer">
            ...
            if (send_email($user['email'], $emailSubject, $emailBody)) {
                $_SESSION['2fa_pending_email'] = true;
                $success = ($formAction === 'resend_2fa_email')
                    ? 'We resent the code to your email.'
                    : 'We emailed a 6-digit code to your address. Enter it below to enable 2FA.';
            } else {
                $errors[] = 'Could not send the email right now. Please try again in a moment.';
            }
        }

    } elseif ($formAction === 'confirm_2fa') {
        $code = trim($_POST['totp_code'] ?? '');
        if (!empty($_SESSION['2fa_pending_email']) && verify_login_otp($user['id'], $code)) {
 ...
 */
function mask_email_for_display_profile($email) {
    ...
    <div class="card">
      ...
      <?php elseif ($pendingEmail2fa): ?>
        ...
        <form method="post">
          ...
          <div class="mb-2"><label class="form-label small">Enter the 6-digit code from your email</label><input type="text" name="totp_code" class="form-control form-control-sm" inputmode="numeric" maxlength="6" pattern="[0-9]{6}" required autofocus></div>
